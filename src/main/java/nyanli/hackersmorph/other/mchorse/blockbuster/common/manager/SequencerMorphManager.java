...
public class SequencerMorphManager {
	...
	static {
		if (Loader.isModLoaded("emoticons")) {
			...
			if (cl instanceof LaunchClassLoader) {
				...
				try {
					supportEmoticons = loader.getClassBytes("mchorse.emoticons.cda04eccaf059893b") != null;
	...
	public static void render(AbstractMorph renderMorph, EntityLivingBase entity, double x, double y, double z, float entityYaw, float partialTicks, SequencerMorph morph) {
		...
		if (prop.repeatTimes > 0 && (times = getRepeatTimes(morph, morph.timer + (morph.isPaused() ? 0 : partialTicks))) > 0) {
			doOffset = true;
			...
			offset = offset.rotateYaw((float) Math.toRadians(-entityYaw));
			GL11.glPushMatrix();
...
//		return extraPropMap.get(threadSide).get(morph);
	...
	public static int getRepeatTimes(SequencerMorph morph, float timer) {
		if (morph.morphs.size() == 0 || morph.isRandom)
			return 0;
		int times = 0;
